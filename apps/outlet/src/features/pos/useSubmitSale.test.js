...
const LINE_ITEMS = [{ product_id: "prod-1", quantity: 2, unit_price: "5.00" }];
...
describe("buildSaleIntent", () => {
  ...
  it("defaults discount_amount and tax_amount to '0.00' when omitted", () => {
    const intent = buildSaleIntent(LINE_ITEMS, {
      outletId: "outlet-1",
  ...
  it("preserves money strings exactly, without parsing them to numbers", () => {
    const intent = buildSaleIntent(
      [{ product_id: "prod-1", quantity: 1, unit_price: "5.10" }],
      {
        outletId: "outlet-1",
        paymentMethod: "card",
        discountAmount: "0.30",
    ...
    expect(intent.payload.line_items[0].unit_price).toBe("5.10");
    expect(intent.payload.discount_amount).toBe("0.30");
